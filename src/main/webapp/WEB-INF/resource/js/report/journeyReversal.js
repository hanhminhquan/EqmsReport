...
function getDataView(formId) {
	...
	setTimeout(function() {
		...
		$.ajax({
			...
			dataType : "json",
			url : path + "/getListJourneyReversal",
	...
	if (dataF != null && dataF.length > 0) {
		for (var i = 0; i < dataF.length; i++) {
			...
			dataFresh[i] = {				
				...
				col1 : dataF[i].customerIc,
				col2 : dataF[i].customerName,		
				col3 : dataF[i].institutionName,
				col4 : dataF[i].workgroupName,
				col5 : dataF[i].visitType,
				col6 : dataF[i].apptTimestamp,
