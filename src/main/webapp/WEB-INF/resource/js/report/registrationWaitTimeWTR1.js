...
function loadDep() {
	...
	var api = "getDepartmentByMemberInstitutionId/";
	//var values = $('#departmentId').val();
	var values =  getSelectedOption(dojo.byId("serviceType"))
	api += values	
	var obj = dojo.byId("departmentId");
	removeAllItem(obj);
	getListDataSelect(api,obj);
	

}


function getSelectedOption(orderButton) {
	var result = [];   
    collection = orderButton.selectedOptions;  
...
function getDataView(formId) {
	...
	setTimeout(function() {
		...
		$.ajax({
			...
			dataType : "json",
			url : path + "/getListRegistrationWaitTimeWTR1",
...
function getParams() {
	...
	var rules = {
		...
		serviceType : dijit.byId("institutionId").get('value'),
...
function resetDataForm() {
	...
	dijit.byId("viewSelection").set('value', 0);
}
...
function relaodData(dataF) {
	...
	if (dataF != null && dataF.length > 0) {
		for (var i = 0; i < dataF.length; i++) {
			...
			//	col8 : '',		
				col12 : dataF[i].registrationTime,
